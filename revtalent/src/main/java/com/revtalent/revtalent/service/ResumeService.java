...
//import com.revtalent.revtalent.repository.mongo.ResumeRepository;
...
//                    .parsedText(extractText(file))   // basic text extraction
//                    .build();
//
...
//                .orElseThrow(() -> new RuntimeException("Resume not found in MongoDB"));
//
//        // NOTE: If you're storing file bytes in MongoDB, add a `byte[] fileData`
//        // field to the Resume model and use it here.
//        // For now returns the parsed text as bytes — replace with actual file bytes.
//        byte[] data = resume.getParsedText() != null
//                ? resume.getParsedText().getBytes()
//                : new byte[0];
//
